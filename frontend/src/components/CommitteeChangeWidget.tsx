        ...
        <div className="flex flex-col md:flex-row items-start gap-5 w-full">
          ...
          {diffReport && (
            <div className="flex-1 w-full space-y-4">
              ...
              {/* Other statistics badges - vertically stacked by default, horizontal on larger screens */}
              <div className="flex flex-col md:flex-row md:flex-wrap gap-3 w-full">
                ...
                {diffReport.new_bills_count !== undefined && diffReport.new_bills_count !== null && (
                  <div className="flex flex-col items-center gap-2 p-3 rounded-lg bg-base-200/30 flex-1 min-w-[120px]">
                ...
                {diffReport.bills_with_new_hearings && (
                  <div className="flex flex-col items-center gap-2 p-3 rounded-lg bg-base-200/30 flex-1 min-w-[120px]">
                    <div className={`badge badge-lg ${diffReport.bills_with_new_hearings.length > 0 ? 'badge-warning' : 'badge-ghost'}`}>
                      <svg className="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                {diffReport.bills_reported_out && (
                  <div className="flex flex-col items-center gap-2 p-3 rounded-lg bg-base-200/30 flex-1 min-w-[120px]">
                    <div className={`badge badge-lg ${diffReport.bills_reported_out.length > 0 ? 'badge-success' : 'badge-ghost'}`}>
                      <svg className="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                {diffReport.bills_with_new_summaries && (
                  <div className="flex flex-col items-center gap-2 p-3 rounded-lg bg-base-200/30 flex-1 min-w-[120px]">
                    <div className={`badge badge-lg ${diffReport.bills_with_new_summaries.length > 0 ? 'badge-success' : 'badge-ghost'}`}>
                      <svg className="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                      </svg>
                ...
                {diffReport.bills_with_new_votes && (
                  <div className="flex flex-col items-center gap-2 p-3 rounded-lg bg-base-200/30 flex-1 min-w-[120px]">
                    <div className={`badge badge-lg ${diffReport.bills_with_new_votes.length > 0 ? 'badge-primary' : 'badge-ghost'}`}>
                      <svg className="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
                      </svg>
                      {diffReport.bills_with_new_votes.length}
                    </div>
